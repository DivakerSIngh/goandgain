...
export class EditCertificationComponent implements OnInit {
  ...
  editCertification() {
    ...
    if (this.isValid()) {
      ...
      body.set('certificationData', JSON.stringify(experienceData));
      body.set('certiDoc', docs.toString());
  ...
  getTeacherInformation(teacherInfo) {
    ...
    if (teacherInfo.certificationDocs != undefined && teacherInfo.certificationDocs.length > 0) {

      for (let i = 0; i < teacherInfo.certificationDocs.length; i++) {
        var fileName = teacherInfo.certificationDocs[i];
        var fileLength = fileName.split('/');
        var NameName = fileLength[fileLength.length - 1];
        if (teacherInfo.certificationDocs[i].length > 0) {
          var obj = {
            fileName: NameName,
            fileLocation: teacherInfo.certificationDocs[i]
          }
          this.uploadValue.push(obj);
        }
      }
    }
      if (teacherInfo.certification!=undefined && teacherInfo.certification.length > 0) {
        this.educationListPrepareForm = [];
        for (let i = 0; i < teacherInfo.certification.length; i++) {
          ...
          var start = teacherInfo.certification[i].startDate.split('-')[0];
          var end = teacherInfo.certification[i].yearOfAcquisition.split('-')[0];
          var YearofAcquisitionList = allYear.filter(x => x >= teacherInfo.certification[i].startDate.split('-')[0]);

          if (start == end) {
            var acquisitionMonthList = allMonth.filter(x => x.id >= teacherInfo.certification[i].startDate.split('-')[1]);
          }
          else {
          ...
          var form = {
            experience: teacherInfo.certification[i].experienceType,
            experienceValid: false,
            title: teacherInfo.certification[i].title,
            titleValid: false,
            institution: teacherInfo.certification[i].institution,
            institutionValid: false,
            location: teacherInfo.certification[i].location,
            locationValid: false,
            stratDateYear: teacherInfo.certification[i].startDate.split('-')[0],
            stratDateYearValid: false,
            stratDateMonth: teacherInfo.certification[i].startDate.split('-')[1],
            stratDateMonthValid: false,
            acquisitionYear: teacherInfo.certification[i].yearOfAcquisition.split('-')[0],
            acquisitionYearValid: false,
            acquisitionMonth: teacherInfo.certification[i].yearOfAcquisition.split('-')[1],
            acquisitionMonthValid: false,
            descriptions: teacherInfo.certification[i].descriptions,
            descriptionsValid: false,
            descriptionLength: 200 - teacherInfo.certification[i].descriptions.length,
            startYearList: this.getStartYear(),
